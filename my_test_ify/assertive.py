...
def equals[
    T
](tested: T, expected: T, id_test: int, description: str = "") -> tuple[
    bool, str, str | None
]:
    if type(expected) != type(tested):
        raise InvalidTypeError(
            type(expected),
            type(tested),
            "Type Error: 'expected' and 'tested' values ​​must be the same type",
        )
...
def greater_than(
    tested: int | float, expected: int | float, id_test: int, description: str = ""
) -> tuple[bool, str, str | None]:
    if not isinstance(tested, (int, float)):
        raise InvalidTypeError(
            type(expected),
            type(tested),
            f"Type Error: the 'tested' value must be int or float, but it is of{type(tested)}",
        )
    if not isinstance(expected, (int, float)):
        raise InvalidTypeError(
            type(expected),
            type(tested),
            f"Type Error: the 'expected' value must be int or float, but it is of{type(expected)}",
        )
